...
| **FTPS sunset comms** | Low | S–M | **Shipped:** blog posts, `/docs/ftp`, FTPS 220 greeting, site-wide **PocketHost 3.0** banner + `/3.0` info page (SFTP, Flounder, pricing preview). **Remaining:** email to active FTPS users, explicit hard removal date, phio/CI SFTP deploy docs. Then schedule **Remove FTPS**. |
...
| **phio SFTP migration (replace FTPS deploy)** | Med | M | **FTPS sunset blocker** — critical path before **Remove FTPS**. **Partial (2026-06-14):** deploy key auto-provision; `dev`/`deploy` now sync via SFTP (`ssh2-sftp-client`, port 2222, email + `Phio` key). **Remaining:** `/docs/ftp` phio + GitHub Actions examples; smoke test in prod. Ship before **Remove FTPS**. |
