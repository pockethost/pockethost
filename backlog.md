...
| **phio SFTP migration (replace FTPS deploy)** | Med | M | **FTPS sunset blocker** — critical path before **Remove FTPS**. phio `dev`/`deploy` still sync via FTPS + `__auth__` cookie. **Partial (2026-06-14):** Ed25519 deploy key auto-provision (`Phio` label, login/info/deploy); `info`/`whoami` merged. **Remaining:** SFTP transport in `vendor/ftp-deploy/` or `ssh2`; `/docs/ftp` phio + GitHub Actions examples; deprecate FTPS in phio output. Ship before **Remove FTPS**. |
