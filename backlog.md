...
| Item | Risk | Effort | Notes |
| ---- | ---- | ------ | ----- |
| **Nightly SQLite vacuum sweep** | Med | M | Nightly job (extend `health-compact`): `VACUUM` instance `data.db` + `logs.db` and Mothership DBs. PB log retention deletes rows but SQLite never shrinks — prod case study: 6.5 GB `logs.db` for ~1.8k rows. Blog: `/blog/pocketbase-sqlite-vacuum`. **Idle/hibernated instances:** external `sqlite3 VACUUM` on disk (container already stopped). **Running instances:** in-process `$app.Vacuum()` / `$app.AuxVacuum()` (blocks DB for duration) or skip until idle. **Mothership:** brief stop window or in-process cron hook. Budget ~1× file size free disk during rewrite. **Before S3 migration** — reclaim bloated sqlite on edge volumes first. Today `health-compact` is a stub (no actual VACUUM). Platform + self-hosters avoid silent disk bloat. |
...
| **SFTP (ssh2) alongside FTPS** | Med | M | Node `ssh2` server on `PH_SFTP_PORT` (default 2222), same email/password auth + virtual FS as FTPS (`InstanceVfs` extracted from `PhFs`). Ship parallel to FTPS (`edge-sftp` PM2 + optional `serve`). Docs/UI: SFTP primary, FTPS legacy. V2: SSH pubkey auth + dashboard key management. |
