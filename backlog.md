...
| Item | Risk | Effort | Notes |
| ---- | ---- | ------ | ----- |
| **Pricing change pre-announcement email** | Low | S–M | **Pre** ([ROADMAP.md](ROADMAP.md)): all users — 3.0, Flounder ends July 1, grandfathering. **Do before** forum/Reddit. Send via `pockethost mail send <messageId>` (`campaigns` / `campaign_messages` + `--confirm`). |
...
| **Last-chance Flounder blast** | Low | S–M | **Post 3.0:** email accounts in **July 1–31 grace window** (registered before July 1). Send via `pockethost mail send`. Not the same as pre-launch urgency blitz. |
...
| **FTPS sunset comms** | Low | S–M | **Phase 1 shipped:** blog, `/docs/ftp`, 220 greeting, site banner, `/3.0`, `/docs/phio`, `/blog/graceful-edge-restarts`. **Post 3.0:** email active FTPS users + explicit hard removal date (`pockethost mail send`) → then **Remove FTPS**. |
...
| **Mothership mailer admin plugin** | Low | M | PB admin plugin (`pb_admin_ext/mailer/`) for bulk mail in superuser dashboard. **Deferred (2026-06-19):** 3.0 ops comms use existing `pockethost mail send` against `campaigns` / `campaign_messages`. Revisit if CLI workflow becomes a bottleneck. |
...
Mothership operator stats ──► ops email audience sizing (SQL in campaign `usersQuery`; optional for CLI sends)
...
| 2026-06-16 | **Instance drawbridge lifecycle** — cache cleared when lowering starts; `ensureInstanceApi` waits out shutdown before raising; gateway pending blocks idle during spawn/proxy |
