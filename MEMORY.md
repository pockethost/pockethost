...
- Instance delete: mothership `DELETE /api/instance/:id` removes the PB record only (after power-off + idle). Edge `edge cleanup` (PM2 `edge-cleanup`, daily): admin `GET /api/instances/data-paths` → `{ id, volume }[]`, then rimraf orphaned dirs under `DATA_ROOT/instances/` (stops bound containers first).
