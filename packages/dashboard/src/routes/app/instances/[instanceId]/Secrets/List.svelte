...
<div class='flex items-center justify-between mb-3 h-9'>
  ...
  <div class="form-control">
    <label class="label cursor-pointer">
      ...
      <input type="checkbox" class="toggle toggle-sm" bind:checked={showSecretKeys} />
    </label>
  </div>
</div>
...
<table class='table'>
  ...
  {#each $items as item}
    <tr transition:fade>
      <th>{item.name}</th>
      <td>{showSecretKeys ? item.value : item.value.slice(0, 2) + item.value.slice(2).replaceAll(/./g, '*')}</td>
      <td class='text-right'>
        <button
          aria-label="Delete"
          on:click={() => items.delete(item.name)}
          type='button'
          class='btn btn-sm btn-square btn-outline btn-warning'><i class="fa-regular fa-trash"></i></button>
      </td>
    </tr>
  {/each}
  </tbody>
