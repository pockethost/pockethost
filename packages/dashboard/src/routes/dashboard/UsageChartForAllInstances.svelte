<script lang="ts">
  ...
  const individualInstanceUsageData = allInstancesArray.map((instance, index) => {
    return {
  ...
  const totalUsageAmount = allInstancesArray.reduce((total, instance) => total + instance.secondsThisMonth, 0);
