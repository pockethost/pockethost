...
export const createAdminPbClient = (url: string, logger: Logger) => {
  ...
  const adminAuthViaEmail = (email: string, password: string) => client.collection("_superusers").authWithPassword(email, password)

  const createFirstAdmin = (email: string, password: string) =>
    client.collection("_superusers").create({ email, password, passwordConfirm: password }).catch((res) => {
